"""
Demo for `adaptive_keyframe_selection.py`.

Builds a synthetic RGB-D sequence with distinct phases so the adaptive behavior
is easy to see, runs the selector, prints a summary, and saves a diagnostic
plot of the error signal vs. the dynamic threshold with the selected keyframes
marked.

Scene / trajectory
------------------
A static, richly-textured wall with a smoothly varying depth map (so that
camera motion induces parallax). Frames are rendered by depth-warping a single
reference frame along a scripted camera trajectory:

  Phase 1  frames  0-14  : camera stationary                (redundant frames)
  Phase 2  frames 15-34  : slow lateral translation         (static scene)
  Phase 3  frames 35-54  : faster translation + small yaw   (static scene)
  Phase 4  frames 55-84  : stationary, but a bright object
                           moves across the view            (dynamic content)

Expected behavior: almost no keyframes while the scene is redundant or while
only the *camera* moves over static geometry (the warp explains that motion);
a burst of keyframes tracking the independently-moving object in Phase 4.
"""
...
def render(tex, depth, pose, K, noise_std=0.012):
    """Render a frame at `pose` by warping the reference (identity) scene.
    Holes (disocclusions) are filled with the reference to mimic an infinite
    static background. A little photometric sensor noise is added so that even
    'redundant' frames are not bit-identical (as with a real RGB-D camera)."""
    ...
    # Mild depth-sensor noise -> reprojection residual that grows with the
    # camera baseline, so faster motion yields larger (but usually sub-
    # threshold) warp error, as happens with a real RGB-D sensor.
...
def add_moving_object(gray, depth, cx, cy, radius=15, brightness=1.0, obj_depth=1.2):
    """Paste a bright circular object (near the camera) at (cx, cy)."""
    H, W = gray.shape
    yy, xx = np.ogrid[:H, :W]
    d2 = (xx - cx) ** 2 + (yy - cy) ** 2
...
def main():
    ...
    selector = AdaptiveKeyframeSelector(
        intrinsics=K,
        alpha=0.7, beta=0.3,        # Eq. 3 weights (paper defaults)
        window_size=5,              # W
        sensitivity=1.5,            # k
