"""
Adaptive Keyframe Selection for Scalable 3D Scene Reconstruction
in Dynamic Environments.

Reference implementation of the method described in:

    R. Jha, Y. Zhou, G. Loianno,
    "Adaptive Keyframe Selection for Scalable 3D Scene Reconstruction in
     Dynamic Environments", arXiv:2510.23928.
...
# =============================================================================
# Depth-based frame warping  (paper Sec. 3.2: "WarpFrame")
# =============================================================================
def warp_frame(
    gray_k: np.ndarray,
    depth_k: np.ndarray,
    ...
    """Forward-warp the keyframe into the current camera view.

    Each keyframe pixel is back-projected to 3D using its depth, transformed
    into the current camera frame via the known poses, and projected into the
    current image plane. A per-pixel z-buffer keeps the nearest surface where
    several source points land on the same target pixel (handles occlusions).

    Args:
        gray_k:   (H, W) keyframe grayscale intensity in [0, 1].
        depth_k:  (H, W) keyframe metric depth.
        pose_k:   (4, 4) keyframe camera-to-world.
        pose_t:   (4, 4) current-frame camera-to-world.
        K:        (3, 3) pinhole intrinsics.
        return_depth: if True, also return the warped surface depth in the
                      current camera.

    Returns:
        warped: (H, W) keyframe intensity resampled into the current view
                (0 where there is no valid correspondence).
        mask:   (H, W) boolean validity mask (True = valid correspondence).
        [wdepth]: (H, W) warped depth, only if return_depth=True.
    """
    H, W = gray_k.shape
...
# =============================================================================
# Error metrics  (paper Sec. 3.2)
# =============================================================================
def photometric_error(cur: np.ndarray, warped: np.ndarray,
                      mask: Optional[np.ndarray]) -> float:
    """Mean-Absolute (L1) photometric error over valid pixels  (Eq. 1).

    L1 is used rather than L2 for robustness to outliers (sensor noise, small
    occlusions)."""
...
def _ssim_map(x: np.ndarray, y: np.ndarray, sigma: float = 1.5,
    ...
    """Per-pixel SSIM between two [0,1] images using a Gaussian window
    (11x11-equivalent, sigma=1.5), following Wang et al. 2004."""
...
def ssim_error(cur: np.ndarray, warped: np.ndarray,
              mask: Optional[np.ndarray]) -> float:
    """Structural dissimilarity 1 - SSIM over the valid region  (Eq. 2).

    Holes in the warped image are filled with the current frame's intensity
    before the windowed SSIM so that hole boundaries do not create spurious
    structural edges; the score is then averaged only over the valid mask."""
    w = warped
    if mask is not None:
        w = warped.copy()
        w[~mask] = cur[~mask]
...
def compute_hybrid_error(
    ...
    """Algorithm 1: warp the keyframe into the current view and return the
    hybrid error e_t = alpha*e_photo + beta*e_ssim, along with the individual
    terms and the validity mask."""
...
class AdaptiveKeyframeSelector:
    """Momentum-aware adaptive keyframe selector (paper Algorithm 2).

    Use it online, one frame at a time:

        sel = AdaptiveKeyframeSelector(K)
        for rgb, depth, pose in stream:
            if sel.update(rgb, depth, pose):
                reconstruction_network.add_keyframe(rgb, depth, pose)

    or in batch over a whole sequence via `run(...)`.

    Hyperparameters (defaults follow the paper, Sec. 3.4):
        alpha, beta   : error weights in Eq. 3            (0.7, 0.3)
        window_size W : sliding window for the statistics (5)
        sensitivity k : control-limit multiplier in Eq. 6 (1.5)
        decay         : refractory strength gamma in Eq. 7 (0.95, in (0,1))
        base_threshold theta0     : floor for theta_t  (dataset-dependent)
        init_threshold theta_init : warm-up threshold before the window fills

    NOTE ON THE REFRACTORY DECAY (Eq. 7).
        The paper motivates the decay factor gamma as a "refractory period" that
        makes it *less likely* for the frame right after a selection to also be
        selected, so that keyframes are spread out during sustained motion. To
        realize that stated behavior, after a selection we transiently *raise*
        the effective threshold and let it relax back toward the adaptive value
        by a factor of gamma each frame:
            refractory <- theta / gamma      (on selection; gamma<1 => raise)
            refractory <- refractory * gamma (every frame; relax back down)
            theta_eff  = max(theta_adaptive, refractory)
        With the paper's gamma = 0.95 a single selection imposes a ~5% higher
        bar for one frame, decaying afterwards. (Taken literally, "theta<-gamma*
        theta" would instead *lower* the threshold and, because theta is
        recomputed from the window every frame, would have no lasting effect;
        we implement the described intent. Set decay=1.0 to disable.)
    """
    ...
    def _adaptive_threshold(self) -> float:
        ...
        # Warm-up phase (Algorithm 2, line 11): blend theta_init -> theta0.
    ...
    def update(self, rgb: np.ndarray, depth: np.ndarray,
        ...
        """Process a single incoming frame; return True if it is selected as a
        keyframe. Populates `self.result` incrementally for diagnostics."""
        self._t += 1
        ...
        # (1) Hybrid error vs. the most recent keyframe (Algorithm 1).
        ...
        # (Algorithm 2, lines 5-8: the current error is part of the window).
        self._window.append(e_t)
        self._error_log.append(e_t)

        # (2) Dynamic threshold with refractory period (Eq. 6 + Eq. 7).
